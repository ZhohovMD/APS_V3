...
void waveform(int argc, char* argv[])
{
    ...
    globalStat.getUserStepVect();
    globalStat.getBufferStepVect();
    globalStat.getExecStepVect();
    ...
    {
        int k = 0;
        for (int j = 0; j < step_+2 && k < globalStat.getUserStepVect().size() - 1; ++j) {
    ...
    for (int i = 0; i < countBuffer; i++)
    {
        ...
        for (int j = 0; j < step_ + 1 && k < globalStat.getBufferStepVect().size() - 1; ++j) {
            if (j == 0)
            {
